...
import 'package:immozen/data/repositories/property_repository.dart';
...
class FetchHomePropertiesCubit extends Cubit<FetchHomePropertiesState> {
  ...
  Future<void> fetchProperty() async {
    try {
      ...
      final result = await _propertyRepository.fetchProperty(
        offset: 0,
      );
      emit(
        FetchHomePropertiesSuccess(
  ...
  Future<void> fetchMoreProperty() async {
    try {
      if (state is FetchHomePropertiesSuccess) {
        ...
        emit(
          (state as FetchHomePropertiesSuccess).copyWith(isLoadingMore: true),
        );
        ...
        final bookingsState = state as FetchHomePropertiesSuccess;
